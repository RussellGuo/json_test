...
void serial_datagram_receive_loop(void *arg)
{
    ...
    for(;;) { // for each datagram
        ...
        if(ret)
        {
            ret = process_outcoming_datagram(decoded_data, decoded_data_len);
            receive_loop_result(decoded_data, decoded_data_len);
        }else{
            return;
        }
    } // for each datagram
...
//    (void)seq;
//    (void)msg_id;
//    receive_loop_result(data_list, len * 4);
