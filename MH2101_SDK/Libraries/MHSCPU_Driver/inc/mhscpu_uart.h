...
#define UART_CLK_SOURCE                (88473600)	
...
typedef struct
{
  uint32_t UART_BaudRate;            /*!< This member configures the UART communication baud rate.
                                           The baud rate is computed using the following formula:
                                            - IntegerDivider = ((UART_CLK_SOURCE) / (16 * (UART_InitStruct->UART_BaudRate)))
...
#define IS_UART_BAUDRATE(BAUDRATE)			(((BAUDRATE) > 0) && ((BAUDRATE) <= (UART_CLK_SOURCE/16)))
