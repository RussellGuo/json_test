...
static  SCI_TypeDef *  const aSCI_BaseTable[EMV_TERMINAL_NUM] = {(SCI_TypeDef *)SCI0_BASE, NULL, (SCI_TypeDef *)SCI2_BASE};
static  const uint8_t        aSCI_CG_Table[EMV_TERMINAL_NUM] = {(uint8_t)SCI0_CG_BIT, NULL, (uint8_t)SCI2_CG_BIT};

volatile struct emv_queue  scs_queue[EMV_TERMINAL_NUM];    /* datas buffer */
static volatile SCI_EMV_PARAM aEMV_Param[EMV_TERMINAL_NUM]; 
...
int32_t SCI_ConfigEMV(uint8_t SCI_Bitmap, uint32_t SCIx_Clk)
{
    ...
    SCIx_RefClock = clocks.PCLK_Frequency / 1000;
    SCIx_Clk *=CLK_DIV;
...
int emv_hard_power_dump( struct emv_core *pdev )
{
    ...
    for (j = 0; j < clocks.PCLK_Frequency / 1000 / 10; j++);
...
int emv_hard_xmit(struct emv_core *pdev)
{
...
#else
    u32TranStep++;
    outputQueue("Send:", pdev->terminal_ch);
    emv_queue_flush(pdev);
    ...
    outputQueue("Recv:", pdev->terminal_ch);
    gu8Step++;
#endif        
...
void emv_hard_VoltageSelect(SCI_VoltageTypeDef SCI_Voltage)
{
    ...
    SYSCTRL->ISO7816_CR = (SYSCTRL->ISO7816_CR & ~(0x3 << 6)) | (SCI_Voltage << 6);
}

void emv_hard_CardDetectSelect(unsigned int SCI_Channel, SCI_CardDetectTypeDef CardDetect)
{
    assert_param(IS_SCI_CARDDETECT(CardDetect));
    
    
    SYSCTRL->PHER_CTRL = (SYSCTRL->PHER_CTRL & ~BIT(16 + SCI_Channel)) | (CardDetect << (16 + SCI_Channel));
}

void emv_hard_VCCENSelect(unsigned int SCI_Channel, SCI_VCCENTypeDef VccEn)
{
    assert_param(IS_SCI_VCCEN(VccEn));
    
    SYSCTRL->PHER_CTRL = (SYSCTRL->PHER_CTRL & ~BIT(20 + SCI_Channel)) | (VccEn << (20 + SCI_Channel));
}
