...
/**
  * @brief  Clear the specified EXTI line pending bits
  * @param  EXTI_Line: specifies the EXTI lines to clear
  *      This parameter can be EXTI_Linex where x can be (0..5).
  * @retval None
  */
...
/**
  * @brief  Clear the specified EXTI pin pending bits
  * @param  EXTI_Line: specify the EXTI line num
  *      This parameter can be EXTI_Linex where x can be (0..5).
  * @param  EXTI_PinSource: specify the EXTI pin num
  *      This parameter can be EXTI_PinSourcex where x can be (0..15).
  * @retval None
  */
void EXTI_ClearPinITPendingBit(uint32_t EXTI_Line, uint32_t EXTI_PinSource)
{
    assert_param(IS_EXTI_LINE(EXTI_Line)); 
	assert_param(IS_EXTI_PIN_SOURCE(EXTI_PinSource)); 
	
    GPIO->INTP_TYPE_STA[EXTI_Line].INTP_STA = 0xFFFF & EXTI_PinSource;
}
