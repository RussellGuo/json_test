...
void OTP_Init(void)
{
    ...
    cycles_1us = (clocks.HCLK_Frequency / 1000 / 1000) + 1;
    
	OTP->TIM = (OTP->TIM & ~0xFF) | cycles_1us;
	
	if (clocks.HCLK_Frequency / 1000 >= 120000)
	{
		rd_cycle = 5;
	}
	else
	...
	if (clocks.HCLK_Frequency / 1000 > 72000)
	{
		rd_byte_wait = 70 / (1000 / (clocks.HCLK_Frequency / 1000 / 1000)) + 1;
	}
	else
	{
		rd_byte_wait = 6;
...
void OTP_PowerOn(void)
{
	...
	SYSCTRL_GetClocksFreq(&clocks);		
	n = clocks.HCLK_Frequency / 1000;
	
	SYSCTRL->LDO25_CR &= ~BIT8;	//打开OTP电源
	while (n--);
