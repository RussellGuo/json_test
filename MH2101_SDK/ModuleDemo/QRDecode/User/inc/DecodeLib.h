...
typedef struct
{
    ...
    //用于GC6153配置，其余sensor无需进行配置
    GPIO_TypeDef*   CAMVirtualHS_GPIO;           /*DCMI 虚拟 HS 输入引脚*/
    uint16_t        CAMVirtualHS_Pin;            /*DCMI 虚拟 HS 输入引脚Pin*/
    GPIO_TypeDef*   CAMVirtualVS_GPIO;           /*DCMI 虚拟 VS 输入引脚*/
    uint16_t        CAMVirtualVS_Pin;            /*DCMI 虚拟 VS 输入引脚Pin*/
    GPIO_TypeDef*   CAMVirtualPCLK_GPIO;         /*DCMI 虚拟 PIXCLK 输入引脚*/
    uint16_t        CAMVirtualPCLK_Pin;          /*DCMI 虚拟 PIXCLK 输入引脚Pin*/
    GPIO_TypeDef*   CAMPixCLK_GPIO;              /*DCMI PIXCLK 输入引脚*/
    uint16_t        CAMPixCLK_Pin;               /*DCMI PIXCLK 输入引脚Pin*/
    GPIO_TypeDef*   CAMData0_GPIO;               /*DCMI Data0 输入引脚*/
    uint16_t        CAMData0_Pin;                /*DCMI Data0 输入引脚Pin*/
}DecodeInitTypeDef;
...
typedef enum
{
    ...
    DecodeInitGPIOError,            /*使用GC6153时，没有进行管脚配置或者管脚配置出错*/
}DecodeFlagTypeDef;
 ...
 *
 */
SensorImageSizeType GetGrayImageSize(void);
/*******************************************************************************/
/**
 * 8W摄像头重新初始化
 *
 */
void Image8WSensorReset(void);

/**
 * dma中断回调函数
 *
 */
void DMA_CallBackFrame(void);

/**
 * 8W镜头数据处理函数
 *
 */
void Image8WDataProcess(void);
/*******************************************************************************/
