...
static const unsigned int kISPB4awbCount = 16;
...
static int minicamera_startpreview(struct minicamera_context *cxt) {
    ...
    zoom_param.mode = 1;
    zoom_param.zoom_level = ZOOM_INFO;
    ...
    SET_PARM(cxt->oem_dev, cxt->oem_handle, CAMERA_PARAM_PREVIEW_FORMAT,
             CAMERA_DATA_FORMAT_YUV420);
    cxt->prev_format = CAMERA_DATA_FORMAT_YUV420;
...
int mini_set_CropRegion(struct minicamera_context *cxt,float zoomRatio,float aspect_ratio){
    ...
    zoom_param.mode = ZOOM_INFO;
    zoom_param.zoom_info.zoom_ratio = zoomRatio;
    zoom_param.zoom_info.prev_aspect_ratio =aspect_ratio;
    SET_PARM(cxt->oem_dev, cxt->oem_handle, CAMERA_PARAM_ZOOM,
             (cmr_uint)&zoom_param);
    flag = 1;    //test
    return 0;
