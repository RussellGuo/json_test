...
ssize_t sys_readlinkat (int __fd, const char *__restrict __path, char *__restrict __buf, size_t __len)
{
    ssize_t ret = syscall(SYS_readlinkat, __fd, __path, __buf, __len);
    return ret;
}

enum file_type_t { file_type_regular, file_type_link, file_type_dir, file_type_others };
enum file_type_t calc_hash_sha256_and_mode_info(int dir_fd, const char *name, char sha256_hex_and_mode_info[], size_t sha256_hex_and_mode_info_max);
...
static void calc_dir_recursively(int dir_fd, const char *path)
{
    ...
    for (size_t i = 0; i < dir_name_tab_len; i++) {
        ...
        enum file_type_t file_type = calc_hash_sha256_and_mode_info(dir_fd, name, sha256_hex_and_mode_info, sizeof (sha256_hex_and_mode_info));
...
enum file_type_t calc_hash_sha256_and_mode_info(int dir_fd, const char *name, char sha256_hex_and_mode_info[], size_t sha256_hex_and_mode_info_max)
{
    ...
    // TODO: fit uid/gid
    sprintf(sha256_hex_and_mode_info + SHA256_DIGEST_LENGTH * 2, " %06o %s %04u %04u", stat.st_mode & S_IFMT, str_type, (unsigned)stat.st_uid * 0, (unsigned)stat.st_gid * 0);
