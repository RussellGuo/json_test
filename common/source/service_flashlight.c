...
static serial_datagram_item_t flashlight_mode, flashlight_mode_param;

// service main loop
__NO_RETURN static void flashlight_thread(void *argument)
{
    (void)argument;
    flashlight_hw_init();
    ...
    for(;;) {
        ...
        flashlight_running_until_new_request_arrived(flashlight_mode, flashlight_mode_param, MODE_CHANGED_FLAG);
    }
}
...
static const osThreadAttr_t ThreadAttr_Flashlight = {
    .name = "Flashlight_Thread",
    .stack_mem  = stack_of_thread,
    .stack_size = sizeof(stack_of_thread),
};


static osThreadId_t tid_flashlight;
...
bool init_thread_of_service_flashlight(void)
{
    ...
    tid_flashlight = osThreadNew(flashlight_thread, NULL, &ThreadAttr_Flashlight);
    return tid_flashlight !=NULL;
}


// ReplyToSetFlashlightConfig, the function is in the API layer of the host-MCU communication
...
void ReplyToSetFlashlightConfig(serial_datagram_item_t mode, serial_datagram_item_t mode_param,
       res_error_code_t *error_code, serial_datagram_item_t seq)
{
    ...
    *error_code = NO_ERROR;
    ...
    case FLASHLIGHT_MODE_FACTORY_TEST:
        break;

    case FLASHLIGHT_MODE_USER1:
    case FLASHLIGHT_MODE_USER2:
