...
//MH2101µÄGPIO×é¡¢¶ÔÓ¦µÄPIN½Å¡¢ÖÐ¶ÏºÅ
#ifdef USE_UART0
...
static void init_uart_controller(UART_TypeDef * uart_no, uint8_t uart_irq);//MH2101µÄ¼¸Â·´®¿ÚÇø·ÖÊ¹ÓÃUART_TypeDef *¶¨Òå£¬ËùÒÔ¸ÄÎªUART_TypeDef *´«²Î
...
bool uart_send_data(const uint8_t *buf, size_t size, const uint32_t delay)
{
    ...
    if (ret) {
        ...
        UART_ITConfig(USART, UART_IT_TX_EMPTY, ENABLE); //MH2101µÄÖÐ¶ÏÉèÖÃº¯Êý Ô­ÐÍÔÚmhscpu_uart.c
    }
...
//¶¨ÒåMH2101 UARTµÄGPIO×é GPIO PIN½Å MODE
...
static inline void uart_irq(UART_TypeDef * uart_no)
{
    ...
    case CHAR_TIMEOUT:
        byte = (uint8_t)UART_ReceiveData(uart_no); //MH2101µÄ´®¿Ú½ÓÊÕº¯Êý Ô­ÐÍÔÚmhscpu_uart.c
    ...
    case TRANS_EMPTY:
        ...
        if (status == osOK) {
            ...
            evt_set_ret = 0;
            UART_SendData(uart_no, byte); //MH2101µÄ´®¿Ú·¢ËÍº¯Êý Ô­ÐÍÔÚmhscpu_uart.c
        } else {
            UART_ITConfig(uart_no, UART_IT_TX_EMPTY, DISABLE); //MH2101µÄÖÐ¶ÏÉèÖÃº¯Êý Ô­ÐÍÔÚmhscpu_uart.c
        }
        break;
    case NONE:
        break;
